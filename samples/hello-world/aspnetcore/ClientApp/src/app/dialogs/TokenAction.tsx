...
const TokenAction: React.FC<ITokenActionProps> = observer((props: ITokenActionProps) => {
  ...
  const onTokenGetClick = async () => {
    const token = await extensibilitySdk.getToken(forced);
    if (token) {
  ...
  return (
    <Dialog onClose={props.onClose} open={props.open}>
      ...
      <DialogContent className={classes.container}>
        ...
        />
        <Button variant="contained" className={classes.actionButton} color="primary" onClick={onTokenGetClick}>
          Fetch token
        </Button>
